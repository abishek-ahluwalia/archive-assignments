...
validation_error = zeros(length(C_all) , length(sigma_all));

for i = 1:length(C_all) 
	for j = 1 : length(sigma_all) 
		model= svmTrain(X, y, C_all(i), @(x1, x2) gaussianKernel(x1, x2, sigma_all(j)));
		predictions = svmPredict(model, Xval);
		validation_error(i , j) = mean(double(predictions ~= yval));
	end
end


[Y , I ] = min(validation_error);
[Y1 , j ] = min(min(validation_error));
i = I(j);
C = C_all(i);
sigma = sigma_all(j);
